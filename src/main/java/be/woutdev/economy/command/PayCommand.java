...
public class PayCommand implements CommandExecutor {
    ...
    private void notEnoughFunds(Player p, OfflinePlayer recipient, BigDecimal amount) {
        if (p.isOnline()) {
            p.sendMessage(String
                .format("%sFailed to transfer %s to %s! You have insufficient funds.", ChatColor.RED,
                    EconomyAPI.getAPI().format(amount),
                    recipient.getName()));
        }
    }
    ...
    private void success(Player p, OfflinePlayer recipient, BigDecimal amount) {
        ...
        if (recipient.isOnline()) {
            ((Player) recipient).sendMessage(String
                .format("%sYou received %s from %s!", ChatColor.GREEN, EconomyAPI.getAPI().format(amount),
                    p.getName()));
        }
    }
}
